        ...
        <tbody>
          <template v-if="detail.length > 0">
            ...
            >
              ...
              <td :data-th="tableTh[4]" class="col-md">{{ item.kind }}</td>
              <td :data-th="tableTh[5]" class="col-md">
                <a :href="item.url" target="_blank">{{ item.serial }}</a>
              </td>
              <template v-if="item.note">
                <td :data-th="tableTh[6]" class="col-md">{{ item.note }}</td>
              </template>
              <template v-else>
                <td :data-th="tableTh[6]" class="col-md noNote">無</td>
              </template>
            </tr>
          </template>
          <template v-else>
